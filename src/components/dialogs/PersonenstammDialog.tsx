...
interface PersonenstammDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Personenstamm['fields']) => Promise<void>;
  defaultValues?: Personenstamm['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function PersonenstammDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: PersonenstammDialogProps) {
  const [fields, setFields] = useState<Partial<Personenstamm['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'personenstamm');
      await onSubmit(clean as Personenstamm['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Personenstamm['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Personenstamm bearbeiten' : 'Personenstamm hinzufügen';
