...
interface PersonenstammViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: Personenstamm | null;
  onEdit: (record: Personenstamm) => void;
}

export function PersonenstammViewDialog({ open, onClose, record, onEdit }: PersonenstammViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Personenstamm anzeigen</DialogTitle>
        </DialogHeader>
