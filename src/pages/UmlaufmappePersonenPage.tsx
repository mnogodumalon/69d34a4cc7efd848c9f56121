...
export default function UmlaufmappePersonenPage() {
  const [records, setRecords] = useState<UmlaufmappePersonen[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<UmlaufmappePersonen | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<UmlaufmappePersonen | null>(null);
  const [viewingRecord, setViewingRecord] = useState<UmlaufmappePersonen | null>(null);
  ...
  async function handleCreate(fields: UmlaufmappePersonen['fields']) {
    await LivingAppsService.createUmlaufmappePersonenEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: UmlaufmappePersonen['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateUmlaufmappePersonenEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteUmlaufmappePersonenEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Umlaufmappe_Personen"
      subtitle={`${records.length} Umlaufmappe_Personen im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Umlaufmappe_Personen suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('aufgabentyp')}>
                <span className="inline-flex items-center gap-1">
                  Aufgabentyp
                  {sortKey === 'aufgabentyp' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.aufgabentyp?.label ?? '—'}</span></TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Umlaufmappe_Personen. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Umlaufmappe_Personen löschen"
