...
import type { Personenstamm } from '@/types/app';
...
export default function PersonenstammPage() {
  const [records, setRecords] = useState<Personenstamm[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Personenstamm | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Personenstamm | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Personenstamm | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getPersonenstamm());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Personenstamm['fields']) {
    await LivingAppsService.createPersonenstammEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Personenstamm['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updatePersonenstammEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deletePersonenstammEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Personenstamm"
      subtitle={`${records.length} Personenstamm im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Personenstamm suchen..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Personenstamm. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
